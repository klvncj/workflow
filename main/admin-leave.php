...
if(isset($_POST['update'])){
    $stat = update_leave_status($_POST);
    if ($stat == true){
...
<body>
                ...
                <div>
                    ...
                    <div class="flex flex-col my-5">
                        ...
                        <span onclick="redirecting('admin-leave.php?u=<?= $admin_id ?>')"
                            class="bg-[#25769b] rounded flex items-center text-align-left p-2 mx-4 text-white  font-light"
    ...
    <div class=" w-full md:ml-32 md:pl-44 drop-shadow-md p-4 md:w-[90%] ">
                ...
                <div>
                    ...
                    <div class=" absolute w-32 right-0 my-5 bg-[#f1f4f5] p-2 rounded drop-shadow-md z-50" id="menus"
                        ...
                        <ul>
                            ...
                            <li><span onclick="move('admin-profile.php?<?= $admin_id ?>')"
        ...
        <div id="leave_request" class="hidden lg:block">
            <h3 class="flex justify-center font-black my-3">Leave Requests</h3>
            <?php
            $leave = show_leave();
            if (!empty($leave)) {
                foreach ($leave as $a_leave) {
                    extract($a_leave); ?>
                    <div>
                        <form method="post" class="flex justify-center p-1">
                            <input type="text" disabled value="<?=$user?>" class="p-1">
                            <input type="text" disabled value="<?=$reason?>">
                            <input type="date" disabled value="<?=$leave_date?>">
                            <input type="date" disabled value="<?=$return_date?>">
                            <input type="hidden" name="id"  value="<?=$leave_id?>">
                            <select name="status">
                                <option <?php if($status == "pending"){echo "selected";}?> value="pending">Pending</option>
                                <option <?php if($status == "Accepted"){echo "selected";}?>  value="Accepted">Accept</option>
                                <option <?php if($status == "Denied"){echo "selected";}?> value="Denied">Denied</option>
                            </select>
                            <button type="submit" name="update" class="bg-green-300 mx-1 p-1 rounded">Update</button>
                        </form>
                    </div>
                <?php }
            } ?>
