...
<style>
    html{
        ...
        transition: scroll-behaviour 0.5s cubic-bezier(0.42, 0, 0.58, 1);
    }
    body {
    ...
    <div class=" w-full md:ml-32 md:pl-44 drop-shadow-md p-4 md:w-[90%] ">
        <nav class="flex justify-between p-2 bg-[#f1f4f5] mb-5 ">
         ...
         <div class="flex items-center">
            <span onclick="move('admin-chat.php?u=<?= $admin_id ?>')" class="flex mx-5 items-center mb-2 mt-2"><i class='bx bx-arrow-back bx-sm'></i></span>
            <div >
                <div class="flex justify-center items-center mt-2 outline outline-2 outline-[#25769b] p-1 m-1 rounded-lg" id="user" onclick="toggleMenu()">
                    <span><i class='bx bxs-user bx-sm'></i></span>
                </div>
                <div class=" absolute w-32 right-0 my-5 bg-[#f1f4f5] p-2 rounded drop-shadow-md z-50" id="menus" style="display: none;">
                    <ul>
                        <li class="flex justify-end" onclick="CloseMenu()"><i class='bx bx-x bx-xs'></i></li>
                        <li><span class="flex justify-start items-center gap-2 p-1 font-semibold text-gray-400 hover:text-blue-500 cursor-pointer duration-700" onclick="toggleFullScreen()"><i class='bx bx-expand bx-xs'></i>Focus</span><hr></li>
                        <li><span onclick="openNav()" class="flex justify-start items-center gap-2 p-1 font-semibold text-gray-400 hover:text-blue-500 cursor-pointer duration-700 md:hidden" onclick="toggleFullScreen()"><i class='bx bx-menu bx-xs'></i>Menu </span><hr></li>
                        <li><span onclick="move('admin-profile.php?u=<?=$admin_id?>')"  class="flex justify-start items-center gap-2 p-1 font-semibold text-gray-400 hover:text-black cursor-pointer duration-700"><i class='bx bxs-cog bx-xs'></i>Settings</span><hr></li>
                        <li><span onclick="LogOut('../Admin/config/logout.php')" class="flex justify-start items-center gap-2 p-1 font-semibold text-gray-400 hover:text-red-500 cursor-pointer duration-700"><i class='bx bx-log-out bx-xs'></i>logout</span></li>
                    </ul>
                </div>
                </span>
            </div>
        ...
        <?php
            ...
            $messages = show_message($name, $user);
            if (!empty($messages)) {
                foreach ($messages as $message) {
                    ...
                    <span
                        class="flex flex-col rounded my-1 bg-<?php if ($user == $sender) {
                            echo "green";
                        } else {
                            echo "blue";
                        } ?>-200 p-5">
                        <div>
                            <span>
                                <p class="font-bold mt-2 text-[#25769b]"><a href="#">
                                        <?php if ($user == $sender) {
                                            echo "Me";
                                        } else {
                                            echo $sender;
            ...
            <form class="flex sticky bottom-0" method="post">
                <input type="hidden" value="<?= $name ?>" name="to">
                <input type="hidden" value="<?= $user ?>" name="from">
                <textarea class="w-80 flex md:w-full items-center" name="message"></textarea>
                <button type="submit" name="send" class="bg-green-500 p-3 rounded ml-1"><i
                        class='bx bxs-paper-plane bx-sm'></i></button>
            </form>
        </div>
    </div>
    <script <script src="./Js/tailwind.js"></script>
