...
if (isset($_POST['send-task'])) {
    $response = admin_task($_POST);
    if ($response === true) {
...
<body>
                ...
                <div>
                    ...
                    <div class="flex flex-col my-5">
                        <span onclick="redirecting('admin-dash.php?u=<?= $admin_id ?>')"
                            class=" flex items-center text-align-left p-2 mx-4  rounded " data-aos="fade-up"
                        ...
                        <span onclick="redirecting('admin-task.php?u=<?= $admin_id ?>')"
                            class="bg-[#25769b] rounded  flex items-center text-align-left p-2 mx-4 text-white"
    ...
    <div class=" w-full md:ml-32 md:pl-44 drop-shadow-md p-4 md:w-[90%] ">
                    ...
                    <div class=" absolute w-32 right-0 my-5 bg-[#f1f4f5] p-2 rounded drop-shadow-md z-50" id="menus"
                        ...
                        <ul>
                            ...
                            <li><span onclick="openNav()"
                                   ...
                                   ><i class='bx bx-menu bx-xs'></i>Menu </span>
                                <hr>
                            </li>
       ...
       <div id="holder" class="grid grid-cols-1 w-[100%] md:grid-cols-2 lg:grid-cols-3 gap-4 mt-12 ">
            <div class=" bg-[#fffafa] drop-shadow-sm flex flex-col p-6">
                <h3 class="font-black">Create To-Do</h3>
                <form method="post">
                <div>
            ...
            <div class=" bg-[#fffafa] drop-shadow-md flex flex-col rounded-lg p-4">
                ...
                if (isset($_GET['user'])) {
                    $user = $_GET['user'];}
                                $histories = show_personal_task('admin');
                                if (!empty($histories)) {
            ...
            <div class=" bg-[#fffafa] drop-shadow-md flex flex-col my-1 rounded-lg p-4 md:w-96">
                <h3 class="font-black">Assign Task To Someone</h3>
                <form method="post" class="flex flex-col">
                    <input type="text" placeholder="Create Task" name="task" class="bg-gray-300 p-1 mt-2 rounded">
                    <select class="bg-gray-300 p-1 mt-2 rounded" name="user">
                        <option selected disabled>Select User</option>
                        <?php
                        $departments = get_all_user();
                        if ($departments) {
                    ...
                    <input type="hidden" name="assigned" value="admin">
                    <input type="submit" name="send-task" class="bg-green-700 p-1 mt-2 rounded text-white" value="Send">
                </form>
            </div>
        </div>
        <div>
            <h3 class="font-black my-5 text-center">Task Assigned</h3>
            <?php 
               $histories = admin_assigned_task();
               if (!empty($histories)) {
                   foreach ($histories as $history) {
                       extract($history); ?>
            <span class="flex flex-col rounded my-2 bg-[#f1f4f5] p-5 ">
                <div class="flex justify-between gap-2">
                    <p class="font-semibold mr-2 text-blue-500 flex items-center"><?=$user?></p>
                    <p id="Last_message" class="font-mono flex items-center"><?=$task?></p><span
                        class="flex gap-4 items-center">
                        <p><?= date('jS, F', strtotime($created_at)); ?></p><i onclick="move('../Admin/config/delete-task-ad.php?id=<?=$task_id?>&user=<?=$firstname?>')" class='bx bxs-x-square bx-xs'></i>
                    </span>
                </div>
            </span>
            <?php }}?>
        ...
        <div class="flex flex-col justify-between p-2  my-5 gap-3 flex-col" id="report-page" style="display: none;">
            <?php
            $report = show_all_report();
            if ($report) {
                foreach ($report as $a_report) {
                    extract($a_report); ?>
                    <span class="flex flex-col rounded my-2 bg-[#f1f4f5] p-3">
                        <div class="flex gap-2 items-center">
                            <span class="flex">
                                <p class="font-bold text-[#25769b]"><a
                                        href="report.php?u=<?=$admin_id?>&report=<?=$report_id?>"><?=$user?> :</a></p>
                            </span>
                            <span class="flex items-center"><a  href="report.php?u=<?=$admin_id?>&report=<?=$report_id?>"><p class="hidden md:block"><?= substr($report, 0, 100) ?>...</p><p class="block md:hidden"><?= substr($report, 0, 20) ?>...</p></a></span>
                        </div>
                    </span>
                <?php }  } ?>
        </div>
