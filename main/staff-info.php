...
?>
            ...
            <span onclick="move('staff.php?u=<?= $admin_id ?>')" class="flex mx-5 items-center mb-2 mt-2"><i class='bx bx-arrow-back bx-sm'></i></span>
            <div >
            <div class="flex items-center">
        ...
        <div>
            <div class="flex justify-start mt-5 gap-6">
                <img src="../Admin/profile/<?= $profile_pic ?>" alt="Add Profile Picture"
                    onclick="Toast('Profile picture')"
                    class="w-24 flex justify-center rounded-[50%] outline outline-3 outline-offset-2 outline-gray-300 aspect-square">
                    <span class="bg-green-400 text-white p-1.5 h-9 mt-7 rounded items-center">
                    <a href="admin-chatroom.php?from=<?= $admin; ?>&to=<?= $firstname; ?>"><i class='bx bxs-paper-plane bx-xs'></i> Message</a>
                    </span>
            </div>
            <div id="name" class="flex flex-col md:flex-row gap-3 mt-5">
                <div class="flex flex-col gap-2"><h4 class="font-black">FULLNAME</h4><span class="bg-gray-300 p-2 w-96 rounded"> <?=$firstname?> <?=$lastname?></span></div>
                <div class="flex flex-col gap-2"><h4 class="font-black">EMAIL</h4><span class="bg-gray-300 p-2 w-96 rounded"><?=$email?></span></div>
            </div>
            <div id="name" class="flex flex-col md:flex-row gap-3 mt-5">
                <div class="flex flex-col gap-2"><h4 class="font-black">PHONE</h4><span class="bg-gray-300 p-2 w-96 rounded"><?=$phone?></span></div>
                <div class="flex flex-col gap-2"><h4 class="font-black">DEPARTMENT</h4><span class="bg-gray-300 p-2 w-96 rounded"><?=$department?></span></div>
            </div>
            <div id="name" class="flex flex-col md:flex-row gap-3 mt-5">
                <div class="flex flex-col gap-2"><h4 class="font-black">POSITION</h4><span class="bg-gray-300 p-2 w-96 rounded"><?=$position?></div>
                <div class="flex flex-col gap-2"><h4 class="font-black">DATE JOINED</h4><span class="bg-gray-300 p-2 w-96 rounded"><?= date('jS F , Y', strtotime($joined_at)); ?></span></div>
            </div>
            <div id="name" class="flex flex-col md:flex-row gap-3 mt-5">
                <button class="bg-red-400 text-white p-1.5 rounded">
                    <a href="../Admin/config/delete.php?id=<?= $staff_id ?>">Remove User</a>
                </button>
            </div>
        </div>
    </div>
