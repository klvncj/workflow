                ...
                <div>
                    ...
                    <div class="flex flex-col my-5">
                        ...
                        <span onclick="redirecting('admin-task.php?u=<?= $admin_id ?>')"
                            class="bg-[#25769b] rounded text-white flex items-center text-align-left p-2 mx-4  font-light" data-aos="fade-up"
    ...
    <div class=" w-full md:ml-32 md:pl-44 drop-shadow-md p-4 md:w-[90%] ">
        <nav class="flex justify-between p-2 bg-[#f1f4f5] mb-5 ">
            ...
            <div class="flex items-center">
                <span onclick="move('admin-task.php?u=<?=$admin_id?>')" class="flex mx-5 items-center mb-2 mt-2"><i
        ...
        if (isset($_GET['report'])) {
            $report_id = $_GET['report'];
                  ...
                  $histories = get_report($report_id);
                  if (!empty($histories)) {
                      foreach ($histories as $history) {
                          extract($history); ?>
        <div id="report-info" class="flex flex-col bg-gray-200 p-3">
            <div id="user" class="flex bg-gray-300 p-2 mb-5 flex-col rounded">
                <span class="flex items-center gap-2"><i class='bx bxs-user-circle bx-sm'></i><?=$user?></span>
                <span class="flex ml-9 text-gray-500"> <?= date('j F Y', strtotime($created_at)); ?></span>
            </div>
